...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const item = await prisma.design.findUnique({ where: { id: params.id } })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      const current = await tx.design.findUnique({ where: { id: params.id }, select: { id: true, order: true } })
      if (!current)
      ...
      if (requestedOrder !== current.order) {
        const siblings = await tx.design.findMany({
        ...
        for (let i = 0; i < newList.length; i++) {
          await tx.design.update({ where: { id: newList[i].id }, data: { order: i + 1 } })
        }
      }
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const current = await tx.design.findUnique({ where: { id: params.id }, select: { id: true } })
      if (!current)
        throw new Prisma.PrismaClientKnownRequestError('Not found', { code: 'P2025', clientVersion: 'unknown' })

      await tx.design.delete({ where: { id: params.id } })

      const remain = await tx.design.findMany({
        orderBy: [{ order: 'asc' }, { createdAt: 'asc' }],
        select: { id: true },
      })
      for (let i = 0; i < remain.length; i++) {
        await tx.design.update({ where: { id: remain[i].id }, data: { order: i + 1 } })
      }
    })
